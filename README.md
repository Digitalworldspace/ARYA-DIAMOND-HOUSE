# Facet — Lab-Grown Diamond & Jewelry Website
...
The site currently ships with placeholder gold-line diamond icons so it looks
finished even before you add real media. Swap them out by adding files with
these exact names (or edit the `src` / `background-image` paths in
`index.html` if you'd rather name them differently):
...
## 2. Personalize the placeholder text
